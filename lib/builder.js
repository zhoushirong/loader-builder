 ...
 * ]
 * ```
 * @param {String} targetDir 生成的目标文件存放目录
 * @param {Array} arr 静态资源数组
 */
exports.minify = function (appDir, targetDir, arr, noDebug) {
  var cache = {};
  var fileCache = {};
  ...
  arr.forEach(function (item) {
    ...
    if (item.type === 'file') {
      ...
      if (!fileCache[item.target]) {
        var file = fs.readFileSync(path.join(appDir, item.target));
        var hashed = md5(file);
        item.min = `${dirname}/${basename}.${hashed}.hashed${extname}`;
        ensureDirectoryExistence(path.join(appDir, item.min))
        fs.writeFileSync(path.join(appDir, item.min), file);
    ...
    item.assets.forEach(function (asset) {
      ...
      // 编译，压缩
      if (!cached) {
        var file = path.join(appDir, asset);
        ...
        try {
          if (asset.endsWith('.min.js') ||
            ...
            var output = exports.processUrl(appDir, targetDir, text, fileCache, item.target);
    ...
    ensureDirectoryExistence(path.join(targetDir, item.min));
    ensureDirectoryExistence(path.join(targetDir, item.debug));
    fs.writeFileSync(path.join(targetDir, item.min), minified);
    if (noDebug !== true) {
      item.debug = `${dirname}/${basename}.${hash}.debug${extname}`;
      fs.writeFileSync(path.join(targetDir, item.debug), content);
    }
...
exports.scanDir = function (dirpath) {
  ...
  views.forEach(function (filename, index) {
    ...
    ensureDirectoryExistence(realPath)
...
exports.checkTarget = function (scaned) {
  ...
  scaned.forEach(function (item) {
    // Loader.file()可以重复
    if (item.type !== 'file' && Object.prototype.hasOwnProperty.call(targets, item.target)) {
...
// url(xxx);
const reg = /(url\(['"]?)([^?)'"#]*)(.*['"]?\))/g;
exports.processUrl = function (appDir, targetDir, text, fileCache, to) {
  return text.replace(reg, function (match, $1, $2, $3) {
    ...
    var basename = path.basename(target, extname);
    var dirname = path.dirname(target);
    var p = path.join(appDir, target)
    ensureDirectoryExistence(p)
    var file = fs.readFileSync(p);
    console.log(file, '3')
    var hashed = dirname + '/' + basename + '.' + md5(file) + '.hashed' + extname;
    ensureDirectoryExistence(path.join(targetDir, hashed))
    fs.writeFileSync(path.join(targetDir, hashed), file);
